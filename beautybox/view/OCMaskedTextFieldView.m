...
-(void)configureTextField
{
    ...
    maskedTextField.layer.masksToBounds=YES;
    maskedTextField.layer.borderColor=[UIColor colorWithWhite:17.0f / 255.0f alpha:0.1f].CGColor;
    maskedTextField.layer.borderWidth= 1.0f;
    maskedTextField.textColor=[UIColor blackColor];
    [maskedTextField setValue:[UIColor blackColor]
        forKeyPath:@"_placeholderLabel.textColor"];
    
    [maskedTextField setFont:[UIFont fontWithName:@"ProximaNova-Light" size:13]];
    
    [maskedTextField setBackgroundColor:[UIColor colorWithRed:1 green:1 blue:1 alpha:1]];
    UIView *paddingView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 20, 20)];
    maskedTextField.leftView = paddingView;
    maskedTextField.rightView=paddingView;
    maskedTextField.leftViewMode = UITextFieldViewModeAlways;
    maskedTextField.rightViewMode = UITextFieldViewModeAlways;
...
- (void)selectTextForInput:(UITextField *)input atRange:(NSRange)range
{
    ...
    //тут надо делегат на текст
    if (self.delegate && [self.delegate respondsToSelector:@selector(didEdit:textField:)])
    {
        [self.delegate performSelector:@selector(didEdit:textField:) withObject:self.getRawInputText withObject:self];
    }
}
